...
frame_time_stamp = 0
pose_landmarks = None
motion_array = np.zeros((DATA_DEPTH, 3*4+2)) 
...
# Open the model
save_motion_scaler = pickle.load(open(os.path.join(SAVE_PATH, 'motion_scaler_30_theta.sav'), 'rb'))
save_motion_pca    = pickle.load(open(os.path.join(SAVE_PATH, 'pca_model_30_theta.sav'), 'rb'))
save_motion_forest = pickle.load(open(os.path.join(SAVE_PATH, 'forest_model_30_theta.sav'), 'rb'))
save_motion_model  = pickle.load(open(os.path.join(SAVE_PATH, 'motion_model_30_theta.sav'), 'rb'))
...
def process_frame_main(frame, depth_frame, motion_array, landmarker, sigma_array):
    ...
    if pose_landmarks:
        ...
        # Renew the data
        motion_array[-1, :12] = np.concatenate((shoulder_to_elbow, 
                                              shoulder_to_wrist, 
                                              shoulder_to_thumb,
        ...
        theta1 = 180 * function_math.normalize_angle(angle=angle1, origin=np.pi) / np.pi
        theta2 = 180 * function_math.normalize_angle(angle=angle2, origin=np.pi) / np.pi
        
        motion_array[-1, 12:] = [theta1, theta2]
...
def display_data(body_position_array, flip_color_image, motion_pred):
    ...
    ################################################################
    cv2.putText(flip_color_image, str(np.round(body_position_array[3,:,1] - body_position_array[2,:,1],2)),  
...
def predict_motion_reduced(motion_array):
    ...
    selector = SelectFromModel(save_motion_forest, threshold="median")
    X_important = selector.transform(flatten_motion_array_reduced)
    print(flatten_motion_array.shape, X_important.shape)
