...
frame_time_stamp = 0
pose_landmarks = None
motion_array = np.zeros((DATA_SIZE, 2*3, len(MOTION_SPEED))) # DATA_SIZE x (3 xyz x 4 points) x 3 speeds
...
def process_frame_train(frame, depth_frame, iter, speed_mode, landmarker, sigma_array):
    ...
    if pose_landmarks:
        ...
        # Renew the data
        motion_array[iter, :, speed_mode] = np.concatenate([shoulder_to_elbow[:2], 
                                                            elbow_to_wrist[:2],
                                                            wrist_to_thumb[:2]])
