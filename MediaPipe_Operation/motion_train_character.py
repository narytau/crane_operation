...
frame_time_stamp = 0
pose_landmarks = None
motion_array = np.zeros((DATA_SIZE, 3*4+2, len(MOTION_SPEED))) # DATA_SIZE x (3 xyz x 4 points) x 3 speeds
...
def process_frame_train(frame, depth_frame, iter, speed_mode, landmarker, sigma_array):
    global pose_landmarks
    global data_window, motion_array, frame_time_stamp, body_position_past
    ...
    if pose_landmarks:
        ...
        motion_array[iter, :12, speed_mode] = np.concatenate((shoulder_to_elbow, 
                                                            shoulder_to_wrist, 
                                                            shoulder_to_thumb,
    ...
    with PoseLandmarker.create_from_options(options) as landmarker:
        ...
        while True:
            ...
            if processed:
                ...
                elbow_to_wrist = body_position_array[3,:,1] - body_position_array[2,:,1]    
                elbow_to_wrist = elbow_to_wrist[:2] / np.linalg.norm(elbow_to_wrist)
                
                angle1 = np.arctan2(wrist_to_pinky[1], wrist_to_pinky[0]) + np.pi
                angle2 = np.arctan2(elbow_to_wrist[1], elbow_to_wrist[0]) + np.pi

                theta1 = 180 * normalize_angle(angle=angle1, origin=np.pi) / np.pi
                theta2 = 180 * normalize_angle(angle=angle2, origin=np.pi) / np.pi
                
                motion_array[iter, 12:, speed_mode] = [theta1, theta2]
...
if __name__ == "__main__":
    ...
    for i in range(3):
        plt.plot(np.linspace(0,DATA_SIZE-1,DATA_SIZE), motion_array[:,i,0])
    plt.legend(['x', 'y', 'z'])
    plt.show()
    ...
    np.savetxt(os.path.join(SAVE_PATH, 'motion_data_30_theta.txt'), data)
    ...
    # Save
    pickle.dump(scaler, open(os.path.join(SAVE_PATH, 'motion_scaler_30_theta.sav'), 'wb'))

    with open(os.path.join(SAVE_PATH, 'pca_model_30_theta.sav'), mode='wb') as f:
        pickle.dump(pca, f)
        
    with open(os.path.join(SAVE_PATH, 'forest_model_30_theta.sav'), mode='wb') as f:
        pickle.dump(forest, f)
        
    with open(os.path.join(SAVE_PATH, 'motion_model_30_theta.sav'), mode='wb') as f:
