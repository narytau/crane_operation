...
frame_time_stamp = 0
pose_landmarks = None
motion_array = np.zeros((DATA_DEPTH, 2*4)) 
...
# Open the model
save_motion_scaler = pickle.load(open(os.path.join(SAVE_PATH, 'motion_scaler_30_angle_900.sav'), 'rb'))
save_motion_pca    = pickle.load(open(os.path.join(SAVE_PATH, 'pca_model_30_angle.sav'), 'rb'))
save_motion_forest = pickle.load(open(os.path.join(SAVE_PATH, 'forest_model_30_angle.sav'), 'rb'))
save_motion_model  = pickle.load(open(os.path.join(SAVE_PATH, 'motion_model_30_angle.sav'), 'rb'))
...
def process_frame_main(frame, depth_frame, motion_array, landmarker, sigma_array):
    ...
    if pose_landmarks:
        ...
        wrist_to_pinky    = function_math.calculate_unit_vector(body_position_array[3,:,1] - body_position_array[2,:,1])
                                
        # Renew the data
        motion_array[-1, :] = np.array([function_math.calculate_theta(shoulder_to_elbow),
...
def predict_motion_reduced(motion_array):
    ...
    # motion_pred, motion_prob = cached_predict_motion(flatten_motion_array_scaled)
