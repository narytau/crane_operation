...
def process_frame_train(frame, depth_frame, iter, speed_mode, landmarker, sigma_array):
    ...
    if pose_landmarks:
        ...
        motion_array[iter, :, speed_mode] = np.concatenate((shoulder_to_elbow, 
                                                            shoulder_to_wrist, 
                                                            shoulder_to_thumb,
...
if __name__ == "__main__":
    ...
    np.savetxt(os.path.join(SAVE_PATH, 'motion_data_30_gaussian.txt'), data)
    ...
    # Save
    pickle.dump(scaler, open(os.path.join(SAVE_PATH, 'motion_scaler_30_gaussian.sav'), 'wb'))

    with open(os.path.join(SAVE_PATH, 'pca_model_30_gaussian.sav'), mode='wb') as f:
        pickle.dump(pca, f)
        
    with open(os.path.join(SAVE_PATH, 'forest_model_30_gaussian.sav'), mode='wb') as f:
        pickle.dump(forest, f)
        
    with open(os.path.join(SAVE_PATH, 'motion_model_30_gaussian.sav'), mode='wb') as f:
