...
# Open the model
save_motion_scaler = pickle.load(open(os.path.join(SAVE_PATH, 'motion_scaler_10_unitvec.sav'), 'rb'))
save_motion_pca    = pickle.load(open(os.path.join(SAVE_PATH, 'pca_model_10_unitvec.sav'), 'rb'))
# save_motion_forest = pickle.load(open(os.path.join(SAVE_PATH, 'forest_model_10_unitvec.sav'), 'rb'))
save_motion_model  = pickle.load(open(os.path.join(SAVE_PATH, 'motion_model_10_unitvec.sav'), 'rb'))
...
def process_frame_main(frame, depth_frame, motion_array, landmarker, sigma_array):
    ...
    if pose_landmarks:
        ...
        # Renew the data
        motion_array[-1, :] = np.concatenate((function_math.calculate_unit_vector(shoulder_to_elbow),
                                              function_math.calculate_unit_vector(shoulder_to_wrist),
                                              function_math.calculate_unit_vector(shoulder_to_thumb),
                                              function_math.calculate_unit_vector(shoulder_to_pinky)))
...
def predict_motion_reduced(motion_array):
    ...
    # print(flatten_motion_array_reduced.shape)
    ...
    motion_pred, motion_prob = cached_predict_motion(flatten_motion_array_reduced)
    
    if max(motion_prob[0]) < 0.95:
        motion_pred = ['NA']
...
def run():
    ...
    with PoseLandmarker.create_from_options(options) as landmarker:
        ...
        while True:
            ...
            motion_pred, motion_prob = predict_motion_reduced(motion_array)
            motion_prob[0][2], motion_prob[0][1] = motion_prob[0][1], motion_prob[0][2]
