...
def process_frame_train(frame, depth_frame, iter, speed_mode, landmarker, sigma_array):
    ...
    if pose_landmarks:
        ...
        # Renew the data
        motion_array[iter, :, speed_mode] = np.array([function_math.calculate_theta(shoulder_to_elbow),
...
if __name__ == "__main__":
    ...
    fig1 = plt.figure()
    for i in range(4):
        plt.plot(np.linspace(0,DATA_SIZE-1,DATA_SIZE), motion_array[:,i,1])
    plt.legend(['theta_1', 'theta_2', 'theta_3','theta_4'])

    fig2 = plt.figure()
    for i in range(4):
        plt.plot(np.linspace(0,DATA_SIZE-1,DATA_SIZE), motion_array[:,i+4,1])
    ...
    np.savetxt(os.path.join(SAVE_PATH, 'motion_data_short.txt'), data)
