...
frame_time_stamp = 0
pose_landmarks = None
motion_array = np.zeros((DATA_SIZE, 3*4, len(MOTION_SPEED))) # DATA_SIZE x (3 xyz x 4 points) x 3 speeds
...
def process_frame_train(frame, depth_frame, iter, speed_mode, landmarker, sigma_array):
    ...
    if pose_landmarks:
        ...
        motion_array[iter, :, speed_mode] = np.concatenate((function_math.calculate_unit_vector(shoulder_to_elbow),
...
if __name__ == "__main__":
    ...
    np.savetxt(os.path.join(SAVE_PATH, 'motion_data_30_unitvec2.txt'), data)
    ...
    # Save
    pickle.dump(scaler, open(os.path.join(SAVE_PATH, 'motion_scaler_30_unitvec.sav'), 'wb'))

    with open(os.path.join(SAVE_PATH, 'pca_model_30_unitvec.sav'), mode='wb') as f:
        pickle.dump(pca, f)
        
    with open(os.path.join(SAVE_PATH, 'forest_model_30_unitvec.sav'), mode='wb') as f:
        pickle.dump(forest, f)
        
    with open(os.path.join(SAVE_PATH, 'motion_model_30_unitvec.sav'), mode='wb') as f:
